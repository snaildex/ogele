...
namespace ogele {
    ...
    void Texture2DArray::Allocate(const ivec2 &newSize) {
        m_size = newSize;
        Bind();
        if (HasMipmaps()) {
	...
	Texture2DArray::Texture2DArray(const ivec2 &size, bool mipmaps, TextureFormat format) :
            TextureBase(TextureType::Single2D, format, mipmaps) {
        Allocate(size);
    }

#define Tex2DArrayDef(dataType, format, pixelDataType) \
    template<> void Texture2DArray::bSetData< dataType >(int level, const ivec2& offset, const ivec2& size, const dataType * data)\
